  ...
  _.extend(Backbone.History.prototype, {
    ...
    navigate : function(fragment, triggerRoute, replace) {
      ...
      if (this._hasPushState) {
		...
		*	CHANGE LOG
		*	@author Luke Moody
		*	Summary: Added support for replacing a history state, rather than
		*	just adding it to the stack automatically.
		*/
		if(replace) {
			window.history.replaceState({}, document.title, loc.protocol + '//' + loc.host + frag);
		}
		else {
		...
		if(replace) {
			var loc = window.location;
			this.fragment = frag;
        	window.location.replace(loc.protocol + '//' + loc.host + '#' + frag);
    	}
		else {
